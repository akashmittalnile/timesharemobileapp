...
export const GooglePlacesAutocomplete = forwardRef((props, ref) => {
  ...
  // const [props.stateText, props.setStateText] = useState('');
  // const [props.dataSource, props.setDataSource] = useState(buildRowsFromResults([]));
  ...
  useEffect(() => {
    ...
    // been rendered
    _handleChangeText(props.stateText);
    return () => {
      _abortRequests();
    };
  ...
  useEffect(() => {
    // Update props.dataSource if props.predefinedPlaces changed
    props.setDataSource(buildRowsFromResults([]));
  }, [props.predefinedPlaces]);

  useImperativeHandle(ref, () => ({
    setAddressText: (address) => {
      props.setStateText(address);
    },
    getAddressText: () => props.stateText,
      ...
      request.onreadystatechange = () => {
        ...
        if (request.status === 200) {
          ...
          if (responseJSON.status === 'OK') {
            ...
            props.setStateText(_renderDescription(rowData));
  ...
  const _disableRowLoaders = () => {
    ...
    props.setDataSource(buildRowsFromResults(_results));
    // }
  };
      ...
      request.onreadystatechange = () => {
        ...
        if (request.status === 200) {
          ...
          if (typeof responseJSON.results !== 'undefined') {
            ...
            props.setDataSource(buildRowsFromResults(results));
            // }
          }
  ...
  const _request = (text) => {
    ...
    if (supportedPlatform() && text && text.length >= props.minLength) {
      ...
      if (props.preProcess) {
        props.setStateText(props.preProcess(text));
      }
  ...
  const _renderSeparator = (sectionID, rowID) => {
    if (rowID === props.dataSource.length - 1) {
      return null;
    }
  ...
  const _shouldShowPoweredLogo = () => {
    if (!props.enablePoweredByContainer || props.dataSource.length === 0) {
      return false;
    }
    ...
    if (
      ...
      return (
        <FlatList
          ...
          extraData={[props.dataSource, props]}
          ItemSeparatorComponent={_renderSeparator}
          renderItem={({ item, index }) => _renderRow(item, index)}
          ListEmptyComponent={
            props.stateText.length > props.minLength && props.listEmptyComponent
          }
          ListHeaderComponent={
            props.renderHeaderComponent &&
            props.renderHeaderComponent(props.stateText)
          }
